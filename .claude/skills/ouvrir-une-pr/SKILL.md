...
| `ruff format scripts .github/scripts .github/assets icone` **puis** `ruff check` sur les mêmes dossiers | dès qu'on touche du **Python**. « Le format » n'est pas une seule chose : `./mvnw spotless:apply` tient le Java, `ruff` tient le Python, et la version est épinglée dans `pyproject.toml` (`ruff==0.16.5`). **Pas de `ruff` sur le poste ?** `python3 -m venv ~/.venv-outils && ~/.venv-outils/bin/pip install ruff==0.16.5`, puis lancer `~/.venv-outils/bin/ruff`. Un `python3 -m pip` qui répond « No module named pip » ne veut pas dire qu'on ne peut pas l'avoir : `ensurepip` en embarque un, et le venv le pose |
