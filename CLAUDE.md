...
Avant la première ligne de code, produire **un seul bloc** - il vaut réservation, résumé et plan à la fois - puis le déposer en commentaire sur l'issue, assigner l'issue, **prévenir les sessions pairs**, et attendre mon accord explicite. Un geste, pas deux : une cérémonie qu'on saute parce qu'elle est longue ne se tiendra pas mieux en s'allongeant.
...
**Et prévenir les pairs, parce que la forge ne réveille personne.** Plusieurs sessions travaillent souvent ce dépôt en même temps, et aucune ne relit les issues des autres avant d'écrire. Le 6 septembre, deux sessions ont pris le même défaut à 23 secondes d'écart, puis une demande a été ouverte 9 secondes avant qu'une autre ne fusionne le même correctif : le commentaire et l'assignee étaient là, et n'ont rien empêché. Un message, lui, arrive tout seul. `ListAgents` dit à qui écrire ; il ne dit pas qui détient une issue, seulement qui est occupé maintenant, et il ne rend pas la même population selon l'endroit d'où on l'interroge. Une prise ne se déduit donc pas d'un listing, elle se déclare.

**La réciprocité est la moitié qui compte** : une annonce reçue vaut prise, et le plan qu'elle décrit n'est pas une liste de tâches à exécuter. Un bloc est précis, donc il tient lieu de mode d'emploi à qui voudrait avancer vite. Si l'on avait déjà commencé, on le dit tout de suite, pas à l'ouverture de la demande.

**Et après une fusion, prévenir aussi** : fusionner déplace la base de tout le monde, en nommant les fichiers touchés pour qu'un pair puisse juger si ça le concerne. Le flux complet est dans les compétences `ouvrir-une-issue` et `clore-une-pr`.
