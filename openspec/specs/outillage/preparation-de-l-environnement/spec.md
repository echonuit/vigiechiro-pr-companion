...
*Vérifié par* : `scripts/methode/prepare-l-environnement.py --auto-test`, onze cas dont quatre
négatifs, appelé par `.githooks/post-checkout` à la création d'un worktree. **Aucun dispositif de CI
ne peut le tenir** : le runner installe tout lui-même et ne joue jamais ce chemin, si bien que
l'auto-test porte seul - il a donc été **vu rouge sur sa propre mutation**, une préparation qui cesse
de poser faisant tomber deux cas.

Le témoin qui l'établit se rejoue : deux worktrees créés sur la même machine, l'un avant ce
changement et l'autre après, où le garde des specs passe de **refus** à **verdict** pour 1 s de plus
sur `git worktree add`.
...
*Vérifié par* : `scripts/batterie.py --auto-test`, quatre cas opposés - un diff de code l'engage, un
diff de test Java aussi, un diff de prose non, un diff vide non plus. Un dispositif qui rendrait
toujours vrai passerait le premier sans rien trier.

Coût mesuré le 2026-09-07 : **20 s** sur un arbre neuf, **9 s** à chaud.
...
*Vérifié par* : `scripts/batterie.py`, dont le récapitulatif rend la ligne de refus de chaque garde
telle qu'il l'a écrite. Aucun cas d'auto-test ne le tient **en propre** : c'est le comportement
antérieur, conservé plutôt qu'ajouté, et le dire vaut mieux que laisser croire à une couverture.
...
*Vérifié par* : `scripts/methode/prepare-l-environnement.py --auto-test`, deux cas - une commande qui
rend un code non nul est comptée sans lever, et un outil **introuvable** du `PATH` est dit en une
ligne plutôt qu'en trace Python. Le second a été trouvé en mesurant, pas en relisant : `npm` absent
faisait sortir une trace entière dans un crochet que personne ne regarde.
...
*Vérifié par* : `scripts/methode/verifie-commandes-prescrites.py`, écrit pour cette exigence. Il
refuse dans les **deux sens** - une distribution déclarée dont le nom ne paraît dans aucune surface de
méthode, et une déclaration que rien ne prescrit, donc morte. Il cherche le **mot entier**, `graphify`
étant une sous-chaîne de `graphifyy` : le garde du nom trompeur s'est fait tromper par un préfixe
