...
| À lancer | Quand |
|---|---|
| `python3 scripts/batterie.py` **puis** `--lance` | **en premier, et avant de composer quoi que ce soit.** Elle dérive ce que CE diff engage depuis le champ `chemins` des contrats, et lance les gardes qui n'en déclarent pas : elle lance trop, jamais trop peu. Elle a été écrite parce que dériver la batterie puis la **filtrer à la main** a laissé passer deux rouges jusqu'en CI le 2026-09-06, et qu'un harnais lançant `python3` sans argument a annoncé 131 commandes vertes sans qu'aucune ne soit jouée. **Elle choisit son interpréteur** : le `.venv` du worktree d'abord, puis celui qui la lance, puis `python3`. Si aucun ne porte les modules que les gardes déclarent, elle **refuse avant le premier garde** en nommant la distribution qui manque, et ce refus parle du poste, pas du diff (ADR 5407, #5434). **Elle joue chaque garde avec les arguments que les ateliers lui donnent**, dérivés de `.github/workflows` : dix gardes que la CI lance avec `--verifie` **écrivent** dans le mode sans argument au lieu de juger, et la porte rendait vert en rendant le dépôt conforme (#5481). Elle joue aussi `ruff`, et sa ligne de verdict dit enfin ce qu'elle n'a PAS joué |
| `ruff format scripts .github/scripts .github/assets icone` **puis** `ruff check` sur les mêmes dossiers | **la porte le fait désormais** (#5481), en 0,02 s, et avec les dossiers que `lint.yml` lui donne. À la main seulement si vous ne passez pas par elle, et alors dès qu'on touche du **Python**. « Le format » n'est pas une seule chose : `./mvnw spotless:apply` tient le Java, `ruff` tient le Python, et la version est épinglée dans `pyproject.toml` (`ruff==0.16.5`). **Pas de `ruff` sur le poste ?** Le `.venv` du worktree le porte, avec tout le groupe `gardes` : `.githooks/post-checkout` le pose à la création (#5426), et `CONTRIBUTING.md` donne le geste à la main. Lancer `.venv/bin/ruff`. Un venv **par worktree**, et non un venv partagé : une branche qui change une version épinglée serait sinon éprouvée contre celle d'une autre |
