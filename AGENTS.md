...
- **Tout travail de branche se fait dans un git worktree dédié.** Ce dépôt-ci
...
- Avant de committer : **`python3 scripts/batterie.py` puis `--lance`**, qui dérive les gardes que CE diff engage et lance ceux qui ne déclarent rien. Elle lance trop, jamais trop peu, et se compose seule : **ne pas se faire un banc à la main**, c'est ce qu'elle remplace. Elle ne couvre PAS deux choses, qui restent à lancer : **tests ciblés** et `./mvnw spotless:apply` (le format Java, que `ruff` ne tient pas). **Le rapport PMD, elle le produit** quand le diff porte du `.java` (#5405), en 20 s, parce que `python3 scripts/adr/4617-code-mort-et-zone-de-test.py` refuse sans lui - et qu'un refus répété se classe « environnemental » alors qu'il peut porter un vrai dépassement de seuil. PMD **produit** le rapport, il ne juge pas : le verdict est ce cliquet, qui compte production et test séparément. Ne pas employer `pmd:check`, qui refuse tout dès la première violation et ne peut pas passer sur ce dépôt. Le profil `-Pquality-gate` rend PMD bloquant sur **toute** violation et ne passe pas non plus : la CI reste autoritaire.
...
Le tour se répète à **chaque petit pas** jusqu'à ce que le comportement soit complet : une issue en compte des dizaines. Le test **avant** le code ; sur un défaut, le premier test **reproduit** le défaut. Un rouge **inattendu** est une trouvaille : lire le message avant de corriger, **puis lui ouvrir une issue tout de suite** - rattachée à l'EPIC du chantier si elle est dans son périmètre, sinon à l'EPIC des suites (#4562). Pas à la clôture, pas en commentaire, et un compte rendu ne compte pas : ce qui se perd est le détail qui la rend traitable, le chiffre et le message littéral. Si le rouge dure, le pas était trop gros : revenir au dernier vert et couper en deux.
...
3. **Doc développeur** : mettre à jour le site dev (`dev-docs/` : architecture, patterns, injection, « ajouter une fonctionnalité »…). Chercher aussi ce qui est devenu FAUX, pas seulement ce qu'il y a à ajouter : une page qui décrit fidèlement un mécanisme remplacé ne rougit nulle part et se lit comme vraie (#3439 a livré les masques dérivés, `captures.md` a décrit « seize » fichiers pendant une semaine). Partir des fichiers touchés et chercher qui les cite dans **deux** corpus : les pages de description (`docs/`, `dev-docs/`, `brief/`) **et les surfaces d'instruction** (`AGENTS.md`, `CLAUDE.md`, `CONTRIBUTING.md`, `.agents/skills/`). Une page qui ment égare ; une surface d'instruction qui ment fait AGIR, chaque session, sans rien faire rougir : #5294 a livré la porte `scripts/batterie.py` et laissé `AGENTS.md` prescrire la liste qu'elle remplace. Les ADR s'écrivent en passe 11, pas ici.
...
10. **Changement OpenSpec archivé** : les delta specs fusionnent d'abord dans `openspec/specs/`, toutes les capacités sont re-comparées, **puis** le dossier passe sous `openspec/changes/archive/`. L'ordre inverse laisse le changement archivé et la spécification jamais mise à jour, sans que rien ne le dise.
11. **ADR du chantier écrites** (`dev-docs/decisions/`), en balayant les passes 0 à 10. Ici et non en passe 3, parce que les passes 4 à 9 produisent des décisions : les cinq ADR du chantier #3151 portent toutes « suite de », aucune n'était née à l'endroit prévu. Le numéro est celui de l'issue. Chaque ADR déclare son article et son niveau de vérification dans son en-tête OKF : `article: A<n>` et `verification: certaine | probable | humaine`, gagé par `enforced_by:` pour les deux premiers et par `loupe:` pour le troisième. Une décision de ne pas faire est une décision, et c'est celle qu'on oublie : elle ne laisse pas de code derrière elle.
12. **Bilan** : synthèse du chantier (ce qui a été livré, dette restante, décisions prises) déposée dans l'EPIC ; les décisions renvoient aux ADR de la passe 11 plutôt que d'en redérouler le raisonnement. Et il se montre : produire un artefact visuel avant / après (une ligne par conséquence visible, la capture des deux états, la phrase qui dit ce qu'on doit y voir), soumis avant de clore l'EPIC : son objet est d'obtenir un assentiment, pas de documenter une décision déjà prise. La passe 8 a ouvert et recadré toutes ces captures ; sans cet artefact, ce travail reste dans la tête de qui l'a fait, et le bilan écrit demande qu'on le croie là où une capture le montre. Les défauts trouvés en chemin y figurent recadrés et agrandis (un glyphe de 12 px ne se juge pas à l'échelle 1), et ce qui n'a pas été corrigé y a sa place : une troncature montrée et assumée vaut mieux qu'une omission.
...
Et les pièges de `grep` qui ont déjà menti ici : un sous-motif se compte lui-même
