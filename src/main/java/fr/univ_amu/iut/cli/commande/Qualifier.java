...
/// `qualifier` (#617) : pose le **verdict de vérification** d'un passage (`ok` / `douteux` / `a-jeter`),
...
@Command(name = "qualifier", description = "Pose le verdict de vérification d'un passage (ok / douteux / a-jeter).")
public final class Qualifier implements Callable<Integer> {
    ...
    @Option(
            ...
            required = true,
            paramLabel = "<verdict>",
            description = "Verdict de vérification : ok, douteux ou a-jeter.")
    ...
    /// Traduit le mot-clé CLI (`ok` / `douteux` / `a-jeter`, insensible à la casse) en [Verdict]. Le verdict
    /// initial `À vérifier` n'est volontairement pas exposé (il n'a de sens qu'avant la première écoute).
    ///
    ...
    static Verdict verdictDepuis(String motCle) {
        return switch (motCle.toLowerCase(Locale.ROOT)) {
            case "ok" -> Verdict.OK;
            case "douteux" -> Verdict.DOUTEUX;
            case "a-jeter", "ajeter" -> Verdict.A_JETER;
            default ->
                throw new ErreurUsage(
                        "Verdict inconnu : « " + motCle + " ». Valeurs acceptées : ok, douteux, a-jeter.");
        };
    }
