...
| [lint.yml](https://github.com/echonuit/vigiechiro-pr-companion/blob/main/.github/workflows/lint.yml) · job `lint` | push `main` + PR | Le **lint au sens propre** : Spotless, ruff, shellcheck, et `test-compile pmd:pmd` qui **produit** le rapport | **Oui** |
| [lint.yml](https://github.com/echonuit/vigiechiro-pr-companion/blob/main/.github/workflows/lint.yml) · job `methode` | push `main` + PR | Les **gardes de méthode** : la prose, les inventaires, les cliquets ADR qui jugent le rapport PMD, les concordances du cycle | **Oui** |
