...
| `scripts/adr/4395-renvois-en-javadoc.py` | le nombre de renvois `#N` portés par la javadoc de production **ne descend pas**. C'est un PLANCHER et non un cliquet : sa polarité est inversée, descendre est la perte. Il existe parce que la résorption de la javadoc coupe la prose où vivent ces citations, et qu'un renvoi perdu ne casse rien - il cesse d'ouvrir. Plancher à 4 076, article A30 | `lint.yml` |
...
Ces gardes sont elles-mêmes du shell, et le dépôt en compte **41 scripts** hors `node_modules`. Le
