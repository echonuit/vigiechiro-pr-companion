...
Les <!--inv:tests-bats-->118<!--/inv--> tests bats visaient le fat-jar par `java -cp` jusqu'à #4071,
...
| `loupe-4712-lots-multi-pr.py` | les lots de chaque EPIC ouvert, mis sous les yeux pour que la question du sous-chantier se pose ; elle ne tranche pas, le nombre de PR qu'un lot portera étant un jugement. **Hors CI** : elle lit la forge, et sans `gh` elle sort en 2 plutôt que de rendre un rapport vide (ADR 4712) | à la main, au découpage d'un chantier et à la passe 9 d'une clôture (autotest : `lint.yml`) |
...
| `scripts/methode/temoins-de-methode-non-decoratifs.py` | qu'aucun `--auto-test` de garde de méthode ne reste vert quand le garde perd sa détection - l'article A2 rendu mécanique pour `scripts/methode/`, comme `verifie_temoins_non_decoratifs.py` le fait pour `scripts/adr/`. La neutralisation s'**insère** avant le point d'entrée et non en fin de fichier, et la fonction d'auto-test en est épargnée. **Refuse** de conclure sur les six gardes sans point d'entrée, qu'il nomme (#4788) | `lint.yml` (autotest dédié) |
...
| `verifie-conditions-booleennes.sh` | une entrée `type: boolean` n'est jamais comparée à une **chaîne** dans un `if:` : `== 'true'` vaut toujours faux, `!= 'true'` toujours vrai, et le témoin « sans gestionnaire de fenêtres » installait donc quand même openbox | `lint.yml` |
| `verifie-conditions-de-job.sh` | la condition d'un job appuyé sur un `needs` porte une **fonction d'état** : sans elle, GitHub l'enveloppe en `success() && (...)` sur tout le graphe amont, et le saut d'un ancêtre la rend inévaluable. La 2.186.0 a eu son tag et sa Release, puis aucun installeur, sur un run vert ([ADR 4079](decisions/4079-une-condition-de-job-nomme-l-etat-qu-elle-attend.md)) | `lint.yml` |
| `clips-orphelins.sh` | désigne les clips de la pré-version roulante que le **dernier tournage n'a pas produits**, et **refuse un dossier de tournage vide** : `--clobber` ne retire rien, donc un cas renommé laissait son clip en ligne, montrant un comportement disparu | `recette-filmee.yml` (autotest : `lint.yml`) |
