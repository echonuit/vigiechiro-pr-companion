...
| `5278-attente-hors-du-fil.py` | les appels a `Attente.que` dont le predicat lit le graphe de scene depuis le fil du test, la ou `Attente.queSurLeFil` existe depuis #4408 pour le lire sur le fil FX. La regle se derive des lectures de noeuds (`lookup(`, `queryAs`, `getItems()`, `getScene()`, `getChildren()`, `getText()`) et non d'une liste de classes. Il **compte** et ne juge pas : `queSurLeFil` coute un aller-retour par tour de boucle, et un site legitimement laisse en `que` ecrit sa raison. Cliquet a 63, que les lots #5269 et #5279 descendent | `lint.yml` (cliquets ADR, autotest dedie) |
