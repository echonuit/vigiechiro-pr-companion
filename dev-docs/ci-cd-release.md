...
| [tournage-recette.yml](https://github.com/echonuit/vigiechiro-pr-companion/blob/main/.github/workflows/tournage-recette.yml) | **manuel** (`workflow_dispatch`) | Tourne les clips d'**une session**, sur la plateforme de son choix - `ubuntu`, `windows` ou `macos`, le banc en Java pur n'ayant besoin d'aucun écran. Répond à la limite que l'EPIC #4133 se donnait : « à 400 clips, ce serait des heures par passage ». La liste des classes est **dérivée** par `CorrespondanceRecetteTest`, jamais tenue à la main. ⚠️ Il ne **publie pas** : les clips restent en artefact, et `clips-recette` reste alimentée par `recette-filmee.yml` tant que le sort du banc bash n'est pas tranché | — |
