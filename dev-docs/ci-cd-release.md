...
| `scripts/batterie.py` | la **porte d'entrée unique** : ce qu'un diff engage, et ce qu'il n'engage pas. Un garde qui ne déclare pas ses `chemins` est **lancé**, et un garde illisible est signalé puis lancé : le défaut penche du côté coûteux, jamais du côté muet. Elle refuse d'engager zéro garde sur un diff non vide ([ADR 5340](decisions/5340-un-garde-qui-ne-declare-pas-ses-chemins-est-lance.md)) | `lint.yml` |
