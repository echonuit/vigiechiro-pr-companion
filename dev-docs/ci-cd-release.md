...
| [maven.yml](https://github.com/echonuit/vigiechiro-pr-companion/blob/main/.github/workflows/maven.yml) · job `ordre-alternatif` | push `main` + PR | Rejoue la suite en **fork unique et ordre inverse**, réparti en **quatre lots** depuis #5329, la graine tournant d'un passage à l'autre | **Oui** |
| [maven.yml](https://github.com/echonuit/vigiechiro-pr-companion/blob/main/.github/workflows/maven.yml) · job `bats` | push `main` + PR | **E2E CLI bats sur le lanceur empaqueté**, 131 cas. Sorti d'`emballage` en #5301 : il en faisait 80 % du temps, et un cas rouge cachait l'état de l'emballage | **Oui** |
...
| [lint.yml](https://github.com/echonuit/vigiechiro-pr-companion/blob/main/.github/workflows/lint.yml) · job `lint` | push `main` + PR | Le **lint au sens propre** : Spotless, ruff, shellcheck | **Oui** |
| [lint.yml](https://github.com/echonuit/vigiechiro-pr-companion/blob/main/.github/workflows/lint.yml) · job `methode` | push `main` + PR | Les **gardes de méthode** : la prose, les inventaires, les concordances, et `test-compile pmd:pmd` qui **produit** le rapport que le cliquet 4617 juge juste après - le producteur vit avec son consommateur | **Oui** |
| [lint.yml](https://github.com/echonuit/vigiechiro-pr-companion/blob/main/.github/workflows/lint.yml) · job `temoins` | push `main` + PR | Les **bancs de mutation** : chaque garde perd sa détection, sa suite doit rougir. Porté depuis #5365, il ne s'exerce que si le diff touche un garde | **Oui** |
...
| `bats` | l'app-image que `emballage` verse | que la CLI répond **depuis le lanceur livré**, sur 131 cas de shell |
...
| `loupe-5539-echeances-non-tenues.py` | les **engagements datés** qu'une issue n'a pas tenus, dans les deux sens : ouverte et échue, ou fermée avant sa propre date. Le signal n'est pas la date mais le **verbe** qui engage : 45 issues ouvertes portent une date et 56 de ces dates sont passées, presque toutes des dates de mesure. Une ligne qui nomme une AUTRE issue est écartée, car elle parle de l'échéance d'un autre. Rend `0` en signalant | `adr-rapport.yml` (rapport du lundi) |
...
| `verifie_temoins_non_decoratifs.py` | aucun témoin de `verifie_scripts.py` n'est décoratif : chaque garde perd sa détection, la suite doit rougir (article A2 rendu mécanique). Le corpus se partage en **deux moitiés disjointes** : les gardes que la suite charge, éprouvés en la jouant, et les **autonomes**, éprouvés par leur propre `--auto-test` (#5134). La partition ne tenait que sans portée : sous portée, les deux listes perdaient leur filtre et un garde tombait dans les deux, où la suite qui l'ignore restait verte quoi qu'il arrive, donc « décoratif » à tort (#5484). La mutation porte sur un **arbre jetable** depuis #4700 - `scripts/` copié, le reste lié - donc l'interrompre ne peut plus laisser un garde neutralisé dans le dépôt. **Un rouge MUET ne tient pas** (#5499) : un auto-test qui sort non nul sans rien dire, hormis le `[]` que rend la neutralisation, n'a joué aucun cas, et il est désormais « non concluant » plutôt que « tient ». Six gardes étaient dans ce cas, leur point d'entrée étant lui-même neutralisé ; la neutralisation épargne donc `main`, `auto_test` et `cas_d_auto_test` par **nom exact**, jamais par motif - « auto » et « test » y protégeraient `auto_test_rougit`, qui est sa propre détection | `lint.yml` |
...
| `5188-corpus-shell.py` | ce qui reste en shell, et que la cible des deux langages condamne (ADR 5188). Un **cliquet à <!--inv:cliquet-corpus-shell-->2<!--/inv-->**, polarité descendante : il n'empêche pas qu'un script grossisse, il empêche la seule chose qui rendrait la cible inatteignable, **qu'on en ajoute**. Le script toléré, `lance-test-filme.sh`, est **compté** et non retiré : une tolérance est un délai, et le retirer ferait croire à une dispense. Sa population est ce que `git ls-files` suit, le même angle mort que les autres cliquets plutôt qu'un angle mort différent ici | `lint.yml` |
...
| `installer_paquets.py` | la porte d'installation : elle écarte ce qui est déjà présent, borne et reprend, et câble le cache des `.deb` ([ADR 4034](decisions/4034-les-paquets-passent-par-une-porte.md)) | `lint.yml` (ses cas) et les cinq workflows qui installent |
...
| `scripts/batterie.py` | la **porte d'entrée unique** : ce qu'un diff engage, et ce qu'il n'engage pas. Un garde qui ne déclare pas ses `chemins` est **lancé**, et un garde illisible est signalé puis lancé : le défaut penche du côté coûteux, jamais du côté muet. Elle refuse d'engager zéro garde sur un diff non vide ([ADR 5340](decisions/5340-un-garde-qui-ne-declare-pas-ses-chemins-est-lance.md)). Elle refuse aussi les fichiers neufs hors de l'index : la porte voit leurs chemins, mais les gardes fondés sur `git ls-files` les excluraient encore de leur population | `lint.yml` Depuis #5481 elle joue chaque garde avec les **arguments des ateliers** - dix d'entre eux écrivent dans le mode sans argument au lieu de juger -, elle joue `ruff`, et sa ligne de verdict nomme ce qu'elle n'a PAS joué : les classes Java, les contrôles de titre et de corps, et les scripts de `.github/scripts` qui ne déclarent pas de `CONTRAT` (#5525). |
| `gardes-java-declares.py` | une classe `*Test.java` qui **construit** un chemin vers de la prose est déclarée dans `batterie.GARDES_JAVA`, ou elle fait rougir. La population se **dérive** de l'arbre et la déclaration lui est confrontée : une liste qu'un garde confronte n'est plus une liste ([ADR 5373](decisions/5373-une-liste-qu-un-garde-confronte-est-un-inventaire.md)) | `lint.yml` |
