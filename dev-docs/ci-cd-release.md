...
| `mesure_minutes_par_pr.py` | ce qu'une **demande** paie et attend, par job : minutes **facturées** (lues dans `billable`, arrondi compris), horloge jusqu'au dernier verdict, et **taux de « sans objet »**. Il relève et ne refuse rien ; seul son auto-test tourne en CI. `mesure_duree_portail.py` ne peut pas le remplacer : il lit `main`, où toutes les portées rendent « oui », donc il afficherait zéro gain par construction (chantier #5294) | `lint.yml` |
