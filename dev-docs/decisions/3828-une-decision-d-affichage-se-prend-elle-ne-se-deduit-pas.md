...
Aucun des trois n'était un bug au sens ordinaire : rien ne plantait, aucun test ne rougissait. Ils
...
La règle antérieure disait « `NO_COLOR` donne le dernier mot à l'utilisateur ». Ce n'était pas faux,
...
L'UTC n'est pas un détail de présentation : « le 3 juillet à 19 h » n'est pas la même heure pour
...
**C'est le nom qui empêche de se tromper, pas la discipline.** `LISIBLE`, `QUAND`, `FORMAT_NUIT` -
...
- **Et il m'a repris sur un autre.** J'avais classé le refus du verrou en « phrase » ; son test a
...
- **Le fuseau est un paramètre, jamais `systemDefault()` en dur** dans ce qui se teste :
