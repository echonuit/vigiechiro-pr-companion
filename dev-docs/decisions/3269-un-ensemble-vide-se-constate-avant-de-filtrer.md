...
Une commande qui filtre **constate un ensemble source vide avant d'appliquer le moindre critère**, et
rend alors un résultat vide en **succès**, dans le format demandé.
...
|---|---|---|
| `lister-passages --lieu` | code 0, « Aucun passage enregistré » | sain (#3269) |
| `lister-especes` / `lister-carres --lieu` | code 0, CSV d'en-têtes | sain (#3269) |
| `lister-observations --lieu` | code 2, « Lieux présents : aucun » | **corrigé ici** |
| `exporter-activite --lieu` | code 2, « Lieux présents : aucun » | **corrigé ici** |
| `exporter-sons --lieu` | code 2, « Passage introuvable » | refus **antérieur** et correct ; le cas ne se poserait que sur un passage existant et vide, non éprouvé |
| `solde-saison --lieu` | code 0, « Aucun point suivi » | sain : `FiltresSaison.parLieu` **qualifie**, il ne lève pas |

La dernière ligne est celle qui surprend : la commande ressemble aux autres, mais son filtre ne
désigne pas. **Un filtre ne se juge pas à son nom** - `parLieu` existe dans deux classes, et une seule
refuse.
...
**Faire rendre une liste vide aux critères qui désignent, sur ensemble vide.** Écarté : cela revient à
défaire l'ADR 3082 dans le cas particulier où elle se trompe, au prix de sa règle générale. Un lieu
mal tapé sur une base peuplée doit continuer de refuser.
