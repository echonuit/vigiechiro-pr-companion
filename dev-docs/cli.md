...
| `deposer-vigiechiro` | `--passage <id> [--token <jeton>] [--archives\|--wav]` | #1043 | `DepotVigieChiro.deposer` (moteur **reprenable** #982, téléversement **parallèle** #984). Défaut = `ChoixSourceDepot.pour`, **le même choix que M-Lot** : ZIP si présentes, sinon invite à les générer (étape 2), sinon repli WAV. `--archives`/`--wav` forcent l'un ou l'autre |
...
| `exporter-activite` | `--passage <id> --sortie <fichier> [--tranche 15\|30\|60] [--format csv]` | #2352 | `ServiceActivite.contactsDuPassage` + `AgregationActivite.parEspece` + `ExportActiviteCsv`. Facette **données** de la courbe d'activité (pendant CLI de l'export image de l'IHM), rattachée à la nuit biologique. **Non gouvernée par le flag `activite-nuit`** : l'agrégation est une capacité stable de `analyse`, le flag ne gouvernant que l'accès à la vue |
