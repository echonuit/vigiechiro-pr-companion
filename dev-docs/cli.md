...
| `audit-coherence` | `[--passage <id>] [--gravite <g>] [--categorie <c>] [--contient <texte>] [--json] [--online] [--token <jeton>]` | #1133, #1254, #1347 | `ServiceAuditCoherence` : confronte **disque, base et serveur**. Sans `--passage`, audite tout le workspace ; avec, une seule nuit (utile après l'avoir réparée). `--online` ajoute les constats qui demandent le réseau (dépôts, points). Les trois filtres (#3258) reprennent les puces de l'écran ; ils s'appliquent **avant** le code de sortie, qui décrit donc **ce qui a été imprimé** et non la base entière. `0` ssi aucun constat d'erreur **parmi ce qui reste** |
