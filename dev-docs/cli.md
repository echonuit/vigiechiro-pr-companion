...
| `restaurer` | `--sauvegarde <chemin> [--complet] --confirmer` | #148, #1346 | `ServiceSauvegarde.restaurer` / `restaurerComplet` : remet la base (et, avec `--complet`, les dossiers de son, **remis là où ils étaient**, avec les `root_path` corrigés et un compte rendu de ce qui a changé de place, #2727). **Écrase l'état local** : `--confirmer` est obligatoire. La base courante est mise de côté (`vigiechiro.db.avant-restauration`) |
