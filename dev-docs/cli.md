...
| `lien-espece` | `--code <code Tadarida>` | #1874 | `TaxonDao` et `ConstructeurLienEspece` : URL seule, PNA puis source universelle choisie dans les réglages ; refus si le taxon est inconnu ou sans fiche |
| `lien-participation` | `--passage <id local>` | #1874 | `PortailVigieChiro.pageParticipation` : URL seule, résolue dans les liens locaux ; refus si aucune participation n'est liée |
