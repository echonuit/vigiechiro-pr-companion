...
| `solde-saison` | `[--annee N] [--campagne <nom>] [--lieu <texte>] [--reste-a-faire] [--format texte\|csv\|json]` | #2356 | `ServiceSoldeSaison` (lecture) : ce qu'il reste à faire, point par point, pour une saison (règles R3/R4 restituées). `--lieu` et `--reste-a-faire` sont les **deux filtres de l'écran** « Ma saison » (#3103), portés ici à la clôture de #3092 et lus sur la même écriture (`FiltresSaison`). Ils ne touchent **que la liste des points** : l'en-tête continue d'annoncer le solde de la **saison entière**, comme à l'écran. Un filtre qui ne retient rien le **dit**. Même décompte que l'écran « Ma saison » (service partagé, parité IHM/CLI). `texte` par défaut, `csv`/`json` scriptables |
