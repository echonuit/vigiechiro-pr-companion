...
| `exporter-activite` | `--passage <id> --sortie <fichier> [--tranche 15\|30\|60] [--format csv]` | #2352 | `ServiceActivite.contactsDuPassage` + `AgregationActivite.parEspece` + `ExportActiviteCsv`. Facette **données** de la courbe d'activité (pendant CLI de l'export image de l'IHM), rattachée à la nuit biologique. **Non gouvernée par la fonctionnalité `activite-nuit`** : l'agrégation est une capacité stable de `analyse`, celle-ci ne gouvernant que l'accès à la vue |
