...
Chaque ADR **déclare comment elle est vérifiée** ([ADR 2465](decisions/2465-une-adr-declare-comment-elle-est-verifiee.md)) : les champs `verification: certaine | probable | humaine` et `enforced_by:` de son en-tête OKF, au même titre que `status`, `article` et `chantier`. Une vérification `humaine` déclare une `loupe:` plutôt qu'un applicateur : nommer un script sous `enforced_by:` prétendrait qu'il refuse, quand il ne fait que relever. Un garde-fou fait rougir la CI si elle manque, ou si le test/script nommé n'existe pas. `certaine` nomme un test ou script déterministe ; `probable` nomme un script de suspects et son **cliquet** ; `humaine` donne le motif, et peut adjoindre une **loupe**. Voir la section « Comment une ADR est vérifiée » de l'[index des décisions](decisions/index.md).
