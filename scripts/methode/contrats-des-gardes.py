...
def population(texte: str) -> str:
    ...
    modules = "\n".join(
        l for l in sans_docstring(texte).split("\n") if l[:1] not in (" ", "\t"))
...
def _auto_test() -> int:
    ...
    verifie("le geste d une loupe aussi",
    ...
    with tempfile.TemporaryDirectory() as brut:
        ...
        verifie("un ecart de population est vu",
                any("population" in l for l in trouves), True)
        ...
        (r / "b/scripts/adr/cliquet-apostrophe.py").write_text(
            'from _commun import RACINES, rapporte\nADR = "1"\nrapporte(ADR, "densite ici", [])\n',
            encoding="utf-8")
        verifie("deux gardes identiques ne rendent aucun ecart",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if not (autre / "scripts" / "adr").is_dir():
            raise SystemExit("%s ne porte pas de scripts/adr/ : ce n est pas un arbre comparable." % autre)
        ...
        print("L'appariement se fait par le geste : deux gardes du même sujet sous deux gestes"
