...
def par_classe(fichier: pathlib.Path) -> tuple[str, int, int] | None:
    ...
    commentaire = sum(1 for l in lignes if _commentaire(l.strip()) and l.strip().lstrip("/").strip())
...
def _auto_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        c = classes([r])
        cas.append(("la densite d une classe se mesure",
                    len(c) == 1 and c[0][2] == 30 and c[0][3] == 26))
    ...
    if rates:
        print(f"\n{len(rates)} cas en échec : la loupe ne mesure pas ce qu'elle annonce.", file=sys.stderr)
        return 1
...
def _rend(titre: str, mesures: list[tuple[float, str, int, int]], combien: int) -> list[str]:
    return [f"{densite * 100:5.0f} %  {com:4} com. / {code:4} code  {ou}"
