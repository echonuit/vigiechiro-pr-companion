...
def noeud(i, label, sf, ligne=1, ft='code'):
    if i not in nodes:
        nodes[i] = {'id': i, 'label': label, 'file_type': ft, 'source_file': str(sf),
                    'source_location': f'L{ligne}', 'norm_label': label.lower(), '_origin': 'pont'}
    return i
...
def lier(s, t, rel, ctx, sf, ligne):
    if s == t or (s, t, rel) in vues:
        return 0
    vues.add((s, t, rel))
    edges.append({'relation': rel, 'confidence': 'EXTRACTED', 'confidence_score': 1.0,
...
for p in sorted(wf_dir.glob('*.yml')) + sorted(wf_dir.glob('*.yaml')):
    ...
    nom = next((m.group(1).strip().strip('"\'')
                for ln in lignes if (m := re.match(r'name:\s*(.+)$', ln))), p.name)
...
print(f'D2 : {n_wf} workflows, {n_job} jobs, {n_lien_prof} liens vers un profil Maven, '
