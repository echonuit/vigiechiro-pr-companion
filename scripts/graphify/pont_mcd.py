        ...
        for a in range(len(cibles)):
            for b in range(a + 1, len(cibles)):
                n_rel += lier(
                    ...
                    cibles[b][1],
                    "shares_data_with",
                    f"mcd {cibles[a][0]}-{verbe}-{cibles[b][0]}",
                    p,
                    i,
                )
...
print(
    f"E : {n_ent} entites, {n_rel} associations, {n_lien} rapprochements entite<->classe (INFERRED 0.85)"
)
