...
- Les **CSV d'observations** suivent le format produit par Tadarida : séparateur point-virgule, champs entre guillemets quand nécessaire, encodage UTF-8 (cf. exemples dans [`samples/transformes/`](https://github.com/IUTInfoAix-S201/brief/tree/main/samples/transformes)). Sur une séquence de 5 s ralentie ×10, Tadarida peut produire **plusieurs lignes** (une par espèce distincte identifiée), avec timing début/fin précis dans la séquence.
