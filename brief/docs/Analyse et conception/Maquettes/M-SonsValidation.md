...
> **Parcours couverts** : [P7 - Valider les résultats Tadarida](../Parcours%20utilisateurs/P7%20-%20Valider%20les%20resultats%20Tadarida.md), [P10 - Exporter une bibliothèque de sons de référence](../Parcours%20utilisateurs/P10%20-%20Exporter%20une%20bibliotheque%20de%20sons%20de%20reference.md).
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 730" role="img" aria-label="Maquette M-SonsValidation - ecran Sons et validation, source passage" style="max-width: 100%; height: auto; border: 1px solid #d0d7de; border-radius: 6px; background: #fafbfc;">
  ...
  <text x="54" y="316" class="cell-sec">… 13 autres observations (colonnes FME, Fréq. term., Début, Durée, Votre certitude, Certitude du validateur masquables via ☰ Colonnes ; contexte Date/Heure/Passage/Carré/Point/Commune masqué ici).</text>
...
- **Table des observations** : une ligne par observation ; colonnes **Fichier**, **Proposition Tadarida** (verrouillée visible), **Proba.**, **Votre taxon** (« - » tant que non revue), **Avis du validateur**, **Fréquence**, **Statut**, **⭐ référence**, **💬 commentaire** (clic = édition en popup), **🗣 fil** (nombre de messages), plus **FME**, **Fréq. terminale**, **Début**, **Durée**, **Votre certitude**, **Certitude du validateur** (masquables). Les colonnes de **contexte** (Date/Heure/Passage/Carré/Point/**Commune**) sont **masquées** quand la source est un passage unique, **affichées** sinon : elles y porteraient la même valeur sur toutes les lignes. La **commune** rejoint ce groupe (#3164) - elle se cochait dans la puce « Lieu » sans que la table la montre. Le **nom du carré** a lui aussi sa colonne (#3300), mais **masquée** au départ : la recherche libre retient une ligne sur ce nom, qui ne se voyait que dans la puce - or une puce ne sert pas la recherche libre ([ADR 3151](https://companion-dev.echonuit.fr/decisions/3151-un-ecran-n-offre-pas-ce-qu-il-ne-montre-pas/)). Masquée parce que cette table est la plus dense du produit ; le sélecteur est là pour ça.
...
- **Compte rendu de la publication** : une fois les corrections envoyées, la zone sous le menu affiche un [compte rendu chiffré](M-CompteRendu.md) *(livré, #2358)* qui dit **quelle part** de la revue est arrivée sur la plateforme. Les **trois natures d'écart** y gardent chacune leur part - à compléter, sans ancrage, hors référentiel - parce qu'elles appellent trois gestes différents, et les **refus** sont groupés **par cause** : vingt observations refusées pour la même panne font une ligne, pas vingt. Il **remplace** la restitution textuelle, qui n'avait que cette surface pour consommateur.
